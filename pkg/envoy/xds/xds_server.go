...
type XDSServer interface {
	...
	// When the proxy acknowledges the network policy update, it will result in a subsequent call
	// to the endpoint's OnProxyPolicyUpdate() function. Returns an error if the policy is invalid,
	// and a revert function that can be called to undo the update.
	...
	// RemoveNetworkPolicy removes network policies relevant to the specified endpoint from
	// the set published to L7 proxies, and stops listening for acks for policies on this endpoint.
