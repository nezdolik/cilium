...
type Cache struct {
	...
	mutex               lock.RWMutex
	resourcesInSnapshot xds.Resources
	snapshotCache       cache.SnapshotCache
	logger              *slog.Logger
	hasher              hash.Hash32
}

// SetSnapshot implements cache.SnapshotCache.
func (c Cache) SetSnapshot(ctx context.Context, node string, snapshot cache.ResourceSnapshot) error {
...
func (c Cache) UpdateSnapshot(ctx context.Context, nodeID string, resources xds.Resources) error {
	oldSnapshot, err := c.GetSnapshot(nodeID)
	if err != nil {
		return err
	}
	c.mutex.Lock()
	...
	if c.areDifferentSnapshots(oldSnapshot, newSnapshot) {
		err = c.snapshotCache.SetSnapshot(ctx, nodeID, newSnapshot)
		if err != nil {
			c.logger.Error("Error setting snapshot %v: %q", newSnapshot, err)
			return err
		}
	} else {
		c.logger.Debug("updateXdsSnapshot: Snapshots are identical, skipping update")
	}

	err = c.snapshotCache.SetSnapshot(ctx, nodeID, newSnapshot)
	if err != nil {
		c.resourcesInSnapshot = resources
	}
	return err
}

func (c Cache) DeleteFromSnapshot(ctx context.Context, nodeID string, resources xds.Resources) error {
	c.mutex.Lock()
	defer c.mutex.Unlock()
	currentResources := c.GetAllResources(nodeID)
	// newResources := s.subtractResources(currentResources, resources)
}
...
func (c Cache) ClearSnapshot(node string) {
	...
	c.resourcesInSnapshot = xds.Resources{}
}
...
func (c Cache) GetAllResources(node string) xds.Resources {
	return c.resourcesInSnapshot
}
...
func (c Cache) subtractResources(a xds.Resources, b xds.Resources) xds.Resources {
