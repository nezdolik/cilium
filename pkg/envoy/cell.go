...
// startableXDSServer extends XDSServer with lifecycle methods used internally by the cell.
type startableXDSServer interface {
	XDSServer
...
func newEnvoyXDSServer(params xdsServerParams) (XDSServer, error) {
	...
	var xdsServer startableXDSServer
	...
	if !option.Config.ExternalEnvoyProxy {
		return &onDemandXdsStarter{
			...
			adsMode:                        option.Config.EnableEnvoyADSServer,
