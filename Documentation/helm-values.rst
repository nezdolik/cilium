...
.. list-table::
   ...
   * - :spelling:ignore:`eni.subnetTagsFilter`
     ...
     - list
     - ``[]``
   * - :spelling:ignore:`envoy.adsServerEnabled`
     - Enable the ADS (Aggregated Discovery Service) xDS server for Envoy proxy configuration instead of the default per-resource-type xDS server.
     - bool
     - ``false``
